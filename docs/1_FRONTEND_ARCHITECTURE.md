...
- **Header**: High-contrast FounderOS branding (logo + bold text).
  - **Global Context Switcher**: A headless dropdown at the very top left (above the routing zones) used to switch between isolated Workspaces (e.g., "FounderOS" vs "Memvella").
- **Zones**:
  ...
  - **Zone 3 (Bottom)**: Profile and active status access.
...
A streamlined interface for system governance, organized via a horizontal tab navigation in the page header (to keep the `Leftnav` clean).

- **Tabs Layout**: A top navigation bar containing tabs like `General`, `Team`, `Integrations`, `Billing`, and `Danger Zone`.
- **Team**: A dedicated table and invite interface for managing human collaborators and their BetterAuth RBAC roles (Owner/Contributor). Features robust client-side validation for invites.
- **Billing**: Hard inputs for GBP (£) daily spending limits and warning thresholds.
- **Integrations**: Inputs for API keys (OpenAI, Anthropic, GitHub) with inline `Copy` and `Trash` actions.
- **Danger Zone**: High-contrast, red-accented area for emergency system halts (circuit breakers).

---

## 4. The Review Overlay (Agent-First Review Room)

The most sophisticated component in the application, triggered by clicking an `ApprovalCard` in the Rightnav Inbox. It allows the CEO to review an agent's proposed action without forcing them into a raw developer environment.

- **Layout**: `max-w-5xl`, 60/40 visual split.
- **Left Column (60% - The Preview Pane)**: 
  - **Default View**: A visual preview of the intended change (e.g., a rendered UI block) or a high-level, plain-English summary of backend logic changes.
  - **Manual Override Toggle**: A discreet button that swaps the visual preview for a full `@monaco-editor/react` Web IDE, acting as an emergency rip-cord for technical founders.
- **Right Column (40% - Agent Iteration Chat)**: A mini chat interface bound specifically to the agent proposing the action. It features the agent's avatar, a history of why they proposed the spec, and its own context-bound Input Dock. The CEO uses this dock to request iterations without touching code.
