...
function isPlaybookTaskPayload(value: unknown): value is PlaybookTaskPayload {
  if (!value || typeof value !== "object") return false;
  const task = value as Partial<PlaybookTaskPayload>;
  return (
    typeof task.title === "string" &&
    typeof task.descriptionTemplate === "string" &&
...
export const generateArtifact = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const versionId = await ctx.db.insert("documentVersions", {
      ...
      versionNumber: 1,
      createdAt: now,
      createdBy: `agent: ${args.agentName}`,
      summary: "Generated by FounderOS.",
    });
...
export const completeTask = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (allDone) {
      ...
      const directive = await ctx.db.get(task.directiveId);
      if (directive?.sessionId) {
        const outputs = await ctx.db
          .query("documents")
          .filter((q) => q.eq(q.field("traceId"), task.directiveId))
          .collect();

        const libraryLines = outputs
        ...
        await ctx.db.insert("chatMessages", {
          ...
          agentName: "FounderOS",
          content: [
            `Completed: ${directive.title}`,
            libraryLines.length > 0
              ? `Saved in Library:\n${libraryLines.join("\n")}`
              : "No Library output was saved for this task.",
          ].join("\n\n"),
        });
...
export const dispatchDirective = internalAction({
  ...
  handler: async (ctx, args) => {
    let parsedTasks: PlaybookTaskPayload[];
    try {
      const parsed = JSON.parse(args.taskMatrix) as unknown;
      if (!Array.isArray(parsed) || !parsed.every(isPlaybookTaskPayload)) {
        throw new Error("Invalid playbook task matrix.");
      }
      parsedTasks = parsed;
    } catch {
    ...
    const resolvedTasks: ResolvedTaskPayload[] = [];
    for (const task of parsedTasks) {
      const agent = await ctx.runQuery(internal.engine.getAgentByName, { name: task.assignedAgentId });
      if (!agent) throw new Error(`Assigned worker not found: ${task.assignedAgentId}`);
...
export const executeTask = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (results.length > 0) {
      ragContext = results.map((result, i) => `[Document ${i + 1}: ${result.title}]\n${result.content}`).join("\n\n");
    }
    ...
    const userPrompt =
      "Complete this business task. Return a concise, founder-facing output with outcomes, decisions, next steps, and any approval needed. Do not mention internal levels, planning internals, model names, logs, or implementation details.";
